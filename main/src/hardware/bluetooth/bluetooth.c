...
esp_bd_addr_t remote_device_addr = {0};
void a2dp_source_cb(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param);
...
static void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param)
{
    ...
    if (eir)
    {
        ...
        {
            ESP_LOGI(BT_TAG, "Found a target device, address %s, name %s", bda_str, s_peer_bdname);
...
static void bt_app_av_sm_hdlr(uint16_t event, void *param)
{
    ESP_LOGI(BT_TAG, "%s state: %d, event: 0x%x", __func__, s_a2d_state, event);
...
static void bt_app_av_state_unconnected_hdlr(uint16_t event, void *param)
{
    ...
    case BT_APP_HEART_BEAT_EVT:
    {
        ...
        ESP_LOGI(BT_TAG, "a2dp connecting to peer: %02x:%02x:%02x:%02x:%02x:%02x",
                 bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        esp_a2d_source_connect(s_peer_bda);
        s_a2d_state = APP_AV_STATE_CONNECTING;
        s_connecting_intv = 0;
    ...
    case ESP_A2D_REPORT_SNK_DELAY_VALUE_EVT:
    {
        ...
        ESP_LOGI(BT_TAG, "%s, delay value: %u * 1/10 ms", __func__, a2d->a2d_report_delay_value_stat.delay_value);
        break;
    }
    default:
    {
...
static void bt_app_av_state_disconnecting_hdlr(uint16_t event, void *param)
{
    ...
    {
        a2d = (esp_a2d_cb_param_t *)(param);
        ESP_LOGI(BT_TAG, "%s, delay value: 0x%u * 1/10 ms", __func__, a2d->a2d_report_delay_value_stat.delay_value);
        break;
    }
    default:
    {
...
static void bt_av_hdl_stack_evt(uint16_t event, void *p_param)
{
    ...
    case BT_APP_STACK_UP_EVT:
    {
        ...
        esp_err_t ret = esp_bt_dev_set_device_name(dev_name);
        if (ret != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "%s failed to set device name %s, error = %x", __func__, dev_name, ret);
            return;
        }
        esp_bt_gap_register_callback(bt_app_gap_cb);

        esp_err_t gap_err = esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_GENERAL_DISCOVERABLE);
        if (gap_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao registrar o callback GAP");
            return;
        }

        esp_err_t avrc_err = esp_avrc_ct_init();
        esp_err_t avrc_reg_err = esp_avrc_ct_register_callback(bt_app_rc_ct_cb);
        if (avrc_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao inicializar o perfil AVRCP");
            return;
        }

        if (avrc_reg_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao registrar o callback AVRCP");
            return;
        }
        ...
        esp_err_t a2d_reg_err = esp_a2d_register_callback(&bt_app_a2d_cb);
        esp_err_t a2d_source_reg_err = esp_a2d_source_register_data_callback(bt_app_a2d_data_cb);
        if (a2d_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao inicializar o perfil A2DP");
            return;
        }

        if (a2d_reg_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao registrar o callback A2DP");
            return;
        }

        if (a2d_source_reg_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao registrar o callback de dados A2DP");
            return;
        }

        /* Avoid the state error of s_a2d_state caused by the connection initiated by the peer device. */
        esp_err_t conn_err = esp_bt_gap_set_scan_mode(ESP_BT_NON_CONNECTABLE, ESP_BT_NON_DISCOVERABLE);
        esp_err_t disc_err = esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, 10, 0);
        if (conn_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao configurar o modo de varredura");
            return;
        }
        ...
        if (disc_err != ESP_OK)
        {
            ESP_LOGE(BT_TAG, "Falha ao iniciar a descoberta de dispositivos");
            return;
        }
...
bool bluetooth_init()
{
    ...
    esp_err_t bt_init_err = esp_bt_controller_init(&bt_cfg);
    ...
    esp_err_t bt_enable_err = esp_bt_controller_enable(ESP_BT_MODE_CLASSIC_BT);
    ...
    esp_err_t bluedroid_init_err = esp_bluedroid_init();
    esp_err_t bluedroid_enable_err = esp_bluedroid_enable();

    if (bluedroid_init_err != ESP_OK && bluedroid_enable_err != ESP_OK)
    {
    ...
    return bt_app_work_dispatch(bt_av_hdl_stack_evt, BT_APP_STACK_UP_EVT, NULL, 0, NULL);
}
